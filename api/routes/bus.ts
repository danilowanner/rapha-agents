      ...
      POST: async ({ request }) => {
        try {
          ...
          if (!body.route || !ROUTE_CONFIG[body.route]) {
            return Response.json({ error: "Invalid route. Supported: 26-central" }, { status: 400 });
          }
          ...
          if (!response.ok) {
            return Response.json({ error: `Failed to fetch bus data: ${response.statusText}` }, { status: 500 });
          }
          ...
          return Response.json({ etas, humanReadable });
        } catch (error) {
